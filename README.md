...
- `docs/images/og-image.png` — the social card, generated by `tools/make-og-image.py`; the script is **reusable for every VogelTronics game** (`--boxart`, `--title`, `--subtitle`, `--url`) so the whole catalog shares one look. `--url` defaults to `vogeltronics.com` — the brand root, since each game sits on its own subdomain
